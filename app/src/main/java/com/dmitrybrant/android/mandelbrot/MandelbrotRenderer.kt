...
class MandelbrotRenderer(private val context: Context) : GLSurfaceView.Renderer {
    ...
    fun handlePan(deltaX: Double, deltaY: Double) {
        // Apply translation without zoom - just move the center
        mandelbrotState?.let { state ->
            try {
                val currentRadius = state.radius.toDouble()
                val currentCenterX = state.centerX.toDouble()
                val currentCenterY = state.centerY.toDouble()
                
                // Scale the translation by the current zoom level
                val scaledDeltaX = deltaX * currentRadius
                val scaledDeltaY = deltaY * currentRadius
                
                // Apply pan by updating the center coordinates
                state.setFromStrings(
                    (currentCenterX + scaledDeltaX).toString(),
                    (currentCenterY + scaledDeltaY).toString(),
                    state.radius
                )
            } catch (e: NumberFormatException) {
                // Fallback for high precision - ignore translation for very high precision cases
                println("High precision pan not supported yet")
            }
        }
    }
    
    fun handleZoom(focusX: Double, focusY: Double, scaleFactor: Double) {
        mandelbrotState?.let { state ->
            // Calculate zoom parameters
            val zoomFactor = 1.0 / scaleFactor // Invert because scale > 1 means zoom in
            
            try {
                val currentRadius = state.radius.toDouble()
                val currentCenterX = state.centerX.toDouble()
                val currentCenterY = state.centerY.toDouble()
                val newRadius = currentRadius * zoomFactor
                
                // For zoom in: zoomFactor < 1, for zoom out: zoomFactor > 1
                val dx = focusX * (1.0 - zoomFactor)
                val dy = focusY * (1.0 - zoomFactor)
                
                // Apply zoom with focus point
                val scaledDx = dx * currentRadius
                val scaledDy = dy * currentRadius
                
                state.setFromStrings(
                    (currentCenterX + scaledDx).toString(),
                    (currentCenterY + scaledDy).toString(),
                    newRadius.toString()
                )
            } catch (e: NumberFormatException) {
                // Fallback: just apply simple zoom without focus point for high precision
                try {
                    val currentRadius = state.radius.toDouble()
                    state.setFromStrings(
                        state.centerX,
                        state.centerY,
                        (currentRadius / scaleFactor).toString()
                    )
                } catch (e2: NumberFormatException) {
                    // Very high precision case - skip zoom
                    println("High precision zoom not supported yet")
                }
            }
        }
    }
